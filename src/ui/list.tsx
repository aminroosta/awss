    ...
    return items_filtered.slice(start, start + height()).map((item, i) => {
      ...
      return {
        ...
        values: columns.map(c => {
          ...
          const parts: { snippet: string; fg: RGBA; bg: RGBA; attrs: number }[] = [];
          if (words.length === 0) {
            const syn = c.syn;
            if (syn) {
              ...
              for (const token of tokens) {
                if (token.trim() === '') {
                  parts.push({ snippet: token, fg: itemFg, bg: itemBg, attrs: itemAttrs });
                } else {
                  const synColors = syn(token);
                  parts.push({ snippet: token, fg: synColors.fg || itemFg, bg: synColors.bg || itemBg, attrs: synColors.attrs !== undefined ? synColors.attrs : itemAttrs });
                }
              }
            } else {
              parts.push({ snippet: remaining, fg: itemFg, bg: itemBg, attrs: itemAttrs });
            }
            return parts;
          }
          ...
          while (remaining) {
            ...
            for (const w of words) {
              ...
              if ((indexOf = remaining.toLowerCase().indexOf(w)) !== -1) {
                if (indexOf > 0) {
                  parts.push({ snippet: remaining.slice(0, indexOf), fg: itemFg, bg: itemBg, attrs: itemAttrs });
                }
                parts.push({ snippet: remaining.slice(indexOf, indexOf + w.length), fg: colors().main.v600, bg: colors().main.v100, attrs: TextAttributes.BOLD });
                      ...
                      <Index each={vitem().values[colIndex]}>
                        {(v) =>
                          <text
                            fg={v().fg}
                            bg={v().bg}
                            attributes={v().attrs | (column().attrs?.(vitem().item) || 0)}
                          >{v().snippet}</text>
                        }
                      </Index>
                    </box>
